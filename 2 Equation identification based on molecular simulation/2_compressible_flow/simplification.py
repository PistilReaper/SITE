...
This module :mod:`simplification` provides utility functions for symbolic simplification of individuals, which may
...
def p_symbol(tensor, plasmid_library):
...
def linker_add(*args):
    ...
    if len(args) > 2:
        sum = args.pop()
        while len(args) > 0:
            sum = operator.add(sum, args.pop())
        return sum

def protected_div_symbol(x1, x2):
    if x2 == 0:
        return 0
    return x1 / x2
...
def _simplify_kexpression(expr, symbolic_function_map, plasmid_library):
    """
    ...
    for i in reversed(range(len(expr))):
        ...
        if isinstance(p, Function):
            ...
            if plasmid_library is None: 
                r = sym_func(*reversed(args))
                expr[i] = sp.simplify(r)
            else:
                if sym_func.__name__ == 'p_symbol':
                    r = sym_func(args[0], plasmid_library)
                else:
                    r = sym_func(*reversed(args))
                expr[i] = sp.simplify(r)
    return expr[0]
...
def simplify(genome, plasmid_library=None, symbolic_function_map=None):
    """
    ...
    if isinstance(genome, KExpression):
        return _simplify_kexpression(genome, symbolic_function_map, plasmid_library)
    elif isinstance(genome, Gene):
        return _simplify_kexpression(genome.kexpression, symbolic_function_map, plasmid_library)
    elif isinstance(genome, Chromosome):
        if len(genome) == 1:
            return _simplify_kexpression(genome[0].kexpression, symbolic_function_map, plasmid_library)
        else:   # multigenic chromosome
            simplified_exprs = [_simplify_kexpression(
                g.kexpression, symbolic_function_map, plasmid_library) for g in genome]
