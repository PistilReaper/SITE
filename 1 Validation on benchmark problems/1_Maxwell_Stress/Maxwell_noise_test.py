...
NOISE = 0.05 # noise level, set to 0.05, 0.1, 0.2
...
T_ij = np.zeros((n_points, 3, 3))
E_iE_j_real = np.zeros((n_points, 3, 3))
B_iB_j_real = np.zeros((n_points, 3, 3))
E2_real = np.zeros((n_points,1))
B2_real = np.zeros((n_points,1))

for idx, (x, y, z) in enumerate(points):
    E[idx] = E_field(x, y, z)
    B[idx] = B_field(x, y, z)
    E_iE_j[idx] = np.outer(E[idx], E[idx])
    B_iB_j[idx] = np.outer(B[idx], B[idx])
    E2[idx] = np.dot(E[idx], E[idx])
    B2[idx] = np.dot(B[idx], B[idx])
    delta_ij[idx] = np.eye(3)
    

noise_E = NOISE *np.array([np.std(E[:,0]), np.std(E[:,1]), np.std(E[:,2])])*np.random.randn(n_points, 3)
noise_B = NOISE *np.array([np.std(B[:,0]), np.std(B[:,1]), np.std(B[:,2])])*np.random.randn(n_points, 3)  
...
for idx, (x, y, z) in enumerate(points):
    E_i = E_n[idx]
    B_i = B_n[idx]
    E_iE_j_real[idx] = np.outer(E_i, E_i)
    B_iB_j_real[idx] = np.outer(B_i, B_i)
    E2_real[idx] = np.dot(E_i, E_i)
    B2_real[idx] = np.dot(B_i, B_i)
    T_ij[idx] = epsilon_0 * (E_iE_j_real[idx] - 0.5 * E2_real[idx] * delta_ij[idx]) + (1 / mu_0) * (B_iB_j_real[idx] - 0.5 * B2_real[idx] * delta_ij[idx])

noise_T_ij = NOISE * np.array([np.std(T_ij[:,0,0]), np.std(T_ij[:,0,1]), np.std(T_ij[:,0,2]),
                               np.std(T_ij[:,1,0]), np.std(T_ij[:,1,1]), np.std(T_ij[:,1,2]),
                                np.std(T_ij[:,2,0]), np.std(T_ij[:,2,1]), np.std(T_ij[:,2,2])]).reshape(3,3) * np.random.randn(n_points, 3, 3)
T_ij_n = T_ij + noise_T_ij
Y = T_ij_n

dict_of_variables = {'E_iE_j':E_iE_j_real,
                     'B_iB_j':B_iB_j_real,
                     'delta_ij':delta_ij,
                     'mu_0':mu_0,
                     'epsilon_0':epsilon_0,
...
r = 15 # length of rnc array for plasmid genes
...
def random_float(a, b):
    ...
    num = round(num, 5)
    return num
toolbox.register('rnc_gen', random_float, a=-10, b=10)
toolbox.register('plasmid_gene_gen', gep.GeneDc, 
                 pset=plasmid_pset, head_length=h_plasmid, rnc_gen=toolbox.rnc_gen, rnc_array_length=r)
...
output_type = f'maxwell_{NOISE}'     # Name of the problem
